...
- **Trap (learned in H1.3): a trailing-window average that divides by the fixed window length is wrong until history fills the window.** Dividing by `windowDays` (7) unconditionally counts pre-tracking days as `0` and drags the mean down. The data-sufficiency gate must therefore be *"the method's first-ever event is at least a full window old"* (`hasFullHistory`) — an event count alone is not enough. And because "today" is in progress, a partial day always reads "below": compare **complete days only** (exclude today; compare *yesterday* against a baseline that also excludes today).
